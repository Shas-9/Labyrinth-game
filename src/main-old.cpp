...
#define MAP_BOUNDARY Vector(1500000, 1500000)
...
public:
  ...
  CamTesting(Vector screen_dimensions) {
    ...
    AreaRect map_boundary(0, 0, MAP_BOUNDARY.x, MAP_BOUNDARY.y);
    Obj map_boundary_obj(map_boundary.pos, map_boundary.size);

    // AreaRect map_boundary2(MAP_BOUNDARY.x/2, MAP_BOUNDARY.y/2, MAP_BOUNDARY.x/2, MAP_BOUNDARY.y/2);

    vector<AreaRect> rects;
    generateBoxes(rects, map_boundary, 50000);

    vector<AreaCirc> circs;
    generateCircles(circs, map_boundary, 50000);

    vector<Obj> rect_objects = {};
    vector<Obj> circ_objects = {};

    for (const auto& rect : rects) { rect_objects.push_back(Obj(rect.pos, rect.size)); }
    for (const auto& circ : circs) { circ_objects.push_back(Obj(circ.pos, circ.radius)); }
    ...
    while (window.isOpen()) {
      ...
      for (auto& obj : circ_objects_in_camera) {
        ...
        // if (qt_container_circ.search(AreaCirc(obj->item.pos, obj->item.radius)).size() > 1) obj->item.setFillColor(sf::Color(255, 255, 255));
        if (qt_container_rect.search(AreaCirc(obj->item.pos, obj->item.radius)).size() > 0) circle.setFillColor(sf::Color(255, 255, 255));
        else circle.setFillColor(sf::Color(100, 100, 255));
        // render
        Vector relative_pos = (obj->item.pos - current_camera_pos) * zoom + Vector(SCREEN_X, SCREEN_Y) / 2;
        circle.setPosition(sf::Vector2f(relative_pos.x, relative_pos.y));
        circle.setRadius(obj->item.radius * zoom);
        ScreenManager::getInstance().window_ptr->draw(circle);
      }
      ...
      for (auto& obj : rect_objects_in_camera) {
        ...
        // setFillColor upon collision
        // if (qt_container_rect.search(AreaRect(obj->item.pos, obj->item.dim)).size() > 1) obj->item.setFillColor(sf::Color(255, 255, 255));
        if (qt_container_circ.search(AreaRect(obj->item.pos, obj->item.dim)).size() > 0) rectangle.setFillColor(sf::Color(255, 255, 255));
        else rectangle.setFillColor(sf::Color(100, 100, 255));
        // render
        Vector relative_pos = (obj->item.pos - current_camera_pos) * zoom + Vector(SCREEN_X, SCREEN_Y) / 2;
        rectangle.setPosition(sf::Vector2f(relative_pos.x, relative_pos.y));
        rectangle.setSize(sf::Vector2f(obj->item.dim.x * zoom, obj->item.dim.y * zoom));
        ScreenManager::getInstance().window_ptr->draw(rectangle);
      }
      ...
      // if (AreaRect(test_box.pos, test_box.dim).overlap(AreaCirc(cursor_circ.pos, cursor_circ.radius))) test_box.setFillColor(sf::Color(255, 255, 255));
      // else test_box.setFillColor(sf::Color(255, 100, 0));
      // test_box.render(current_camera_pos, zoom);

      // // if (AreaCirc(test_circ.pos, test_circ.radius).contains(AreaCirc(cursor_circ.pos, cursor_circ.radius))) test_circ.setFillColor(sf::Color(255, 255, 255));
