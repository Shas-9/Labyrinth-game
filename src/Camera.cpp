...
void Camera::updatePos(bool smooth) {
  ...
  if (smooth) {
    Vector new_cam_pos = Vector::getMid(this->current_camera_pos, this->target_camera_pos, 6);
...
void Camera::updateZoom(bool smooth) {
  // smooth camera movement
  if (smooth) {
    this->current_custom_zoom -= (this->current_custom_zoom - this->target_custom_zoom)/6;
  }
